...
const BeaverAgent: React.FC<BeaverAgentProps> = ({ onClose, onIdeaReady, projectTitle }) => {
      ...
      <div className="pointer-events-auto bg-white/95 backdrop-blur-xl border-4 border-amber-400 rounded-t-3xl shadow-2xl w-full max-w-lg p-6 mb-0 animate-slide-up relative">
        ...
        <div className="flex items-center gap-6">
            ...
            <div className="relative">
                <div className={`w-24 h-24 rounded-full border-4 ${isTalking ? 'border-green-500 scale-110' : 'border-amber-400'} bg-amber-100 flex items-center justify-center overflow-hidden transition-all duration-300 shadow-lg`}>
                     {/* CSS Beaver Face Placeholder */}
                     <div className="text-6xl select-none transform transition-transform duration-200" style={{ transform: isTalking ? 'translateY(-2px)' : 'none' }}>
                         🦫
                     </div>
                </div>
                {isTalking && (
            ...
            <div className="flex-1">
                <h3 className="text-xl font-extrabold text-slate-800 mb-1">Benny the Builder</h3>
                <p className="text-sm text-slate-500 font-medium flex items-center">
                    {isConnected ? <span className="w-2 h-2 bg-green-500 rounded-full mr-2 animate-pulse"></span> : <span className="w-2 h-2 bg-slate-300 rounded-full mr-2"></span>}
                    {status}
                </p>
        ...
        <div className="mt-6 bg-slate-100 rounded-xl p-4 flex items-center justify-between border border-slate-200">
             <div className="flex space-x-1 h-8 items-center">
                 ...
                 {[1,2,3,4,5].map(i => (
                     <div key={i} className={`w-1.5 bg-slate-400 rounded-full transition-all duration-100 ${isTalking ? 'bg-amber-500' : ''}`} style={{ height: isTalking ? `${Math.random() * 24 + 8}px` : '8px' }}></div>
                 ))}
             </div>
